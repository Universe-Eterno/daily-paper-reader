...
tldr: 本研究探讨了图神经网络（MPNN）在神经算法推理中的泛化能力，旨在解决现有研究缺乏形式化保证的问题。作者提出了一个通用理论框架，明确了 MPNN 从小规模训练集学习并泛化至任意规模输入的充分条件，涵盖了最短路径、最小生成树及动态规划等算法。此外，研究还揭示了标准 MPNN 的局限性并提出了增强型架构，通过对 Bellman-Ford 算法的深入分析，显著降低了训练数据需求，为构建具备算法推理能力的神经系统提供了理论支撑。
selection_source: fresh_fetch
motivation: 现有的神经算法推理研究多为经验性，缺乏关于图神经网络如何从有限训练集泛化到任意规模输入的理论保证。
method: 提出了一个通用的理论框架，定义了 MPNN 学习并泛化至最短路径、最小生成树及动态规划等算法的充分条件，并针对标准模型的局限性提出了改进架构。
result: 证明了 MPNN 在特定条件下可学习多种经典算法，揭示了标准架构的局限性，并优化了 Bellman-Ford 算法的训练效率与正则化方法。
conclusion: 该研究为 MPNN 的算法推理能力奠定了理论基础，明确了其可学习的算法边界，并为开发更强大的神经算法模型提供了指导。
---
