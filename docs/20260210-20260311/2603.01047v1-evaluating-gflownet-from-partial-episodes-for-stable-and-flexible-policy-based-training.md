...
method: 提出评估平衡目标，利用流量平衡原理构建策略评估器，并支持在部分片段上进行训练。
result: 在合成和真实任务中证明了该方法能提高训练稳定性，并灵活支持参数化后向策略与离线数据。
conclusion: 该研究通过评估平衡目标统一了GFlowNet的两种训练视角，为构建更稳定、灵活的生成流网络提供了理论支持。
---
