...
tldr: 本研究提出双层自动研究框架（Bilevel Autoresearch），利用大模型实现研究过程的自我优化。外层循环作为元优化器，通过生成并注入Python代码来改进内层研究循环的搜索机制。在GPT预训练基准测试中，该系统实现了5倍的性能提升，并自主发现了组合优化、多臂老虎机等复杂机制，展示了AI驱动研究递归进化的潜力。
selection_source: fresh_fetch
motivation: 现有的自动研究系统仍依赖人工改进代码来解决瓶颈，缺乏完全自主的进化能力。
method: 采用双层架构，由外层循环在运行时生成并注入新的搜索机制代码，从而优化内层循环的搜索效率。
result: 在GPT预训练任务中，元优化外层循环比标准内层循环性能提升5倍，并自主发现了多种高级优化策略。
conclusion: 自动研究系统能够通过元优化自身机制来打破搜索模式的局限，实现对复杂目标的高效探索与自我改进。
---
