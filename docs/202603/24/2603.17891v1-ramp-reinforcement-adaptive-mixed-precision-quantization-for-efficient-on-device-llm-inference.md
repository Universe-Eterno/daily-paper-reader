...
tldr: 针对大语言模型在资源受限设备上的部署，本文提出RAMP框架，利用强化学习（Soft Actor Critic）实现自动化的逐层混合精度量化。通过引入Scale Folding预处理技术和多维特征嵌入，RAMP能在满足全局显存预算的同时最小化模型困惑度。该方法支持跨模型架构的零样本迁移，在Llama 2和Mistral等模型上以更低的平均比特率实现了优于AWQ和GPTQ的精度表现，并支持GGUF格式的高效推理。
selection_source: fresh_fetch
motivation: 现有的后训练量化方法多采用统一位宽，无法在模型精度与硬件资源消耗之间达到最优平衡。
method: 提出基于Soft Actor Critic的强化学习框架，结合Scale Folding预处理技术，根据层特征自动分配最优位宽。
result: 在Llama 2 7B上以3.65有效位宽实现了优于4位AWQ的精度，且训练好的策略可直接应用于不同规模和家族的模型。
conclusion: RAMP证明了量化敏感性主要由架构决定，通过自适应混合精度量化可显著提升大模型在边缘设备上的推理效率。
---
