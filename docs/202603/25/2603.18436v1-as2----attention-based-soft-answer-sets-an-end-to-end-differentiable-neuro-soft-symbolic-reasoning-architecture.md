...
title_zh: AS2 -- 基于注意力的软回答集：一种端到端可微的神经-软符号推理架构
...
tldr: 针对神经符号AI中感知与推理模块间不可微的瓶颈，本文提出AS2架构。它通过将Answer Set Programming (ASP) 的算子转化为可微的软近似，实现了端到端的训练。该架构利用约束感知注意力机制和固定点残差最小化，无需外部求解器即可完成复杂逻辑推理。在数独和MNIST加法任务上表现优异，证明了软符号推理在保持可微性的同时能达到极高精度。
selection_source: fresh_fetch
motivation: 解决传统神经符号系统中感知模块与离散符号求解器之间因不可微边界导致无法进行端到端反馈训练的问题。
method: 提出一种基于注意力的软答案集架构，通过模拟ASP算子的连续近似并最小化概率固定点残差，实现无需外部求解器的端到端推理。
result: "在视觉数独任务中达到99.89%的准确率和100%的约束满足率，在多位MNIST加法任务中准确率超过99.7%。"
conclusion: 结合声明式约束规范与可微固定点算子的软推理架构，能够匹配甚至超越基于外部求解器的神经符号系统。
---

## 摘要
神经符号人工智能（AI）系统通常通过不可微边界将神经感知模块与离散符号求解器耦合，这阻碍了约束满足反馈在训练期间传导至感知编码器。我们引入了 AS2（基于注意力的软回答集），这是一种完全可微的神经符号架构，它使用回答集编程（ASP）直接推论算子 $T_P$ 的软连续近似取代了离散求解器。AS2 在整个前向传播过程中维持有限符号域上的逐位置概率分布，并通过最小化 $T_P$ 概率提升的不动点残差来进行端到端训练，从而在训练或推理阶段均无需调用外部求解器即可实现约束检查的可微化。该架构完全不使用传统的位置嵌入。相反，它通过直接反映声明式 ASP 规范的约束组隶属度嵌入来编码问题结构，使模型对任意位置索引具有不可知性。在视觉数独（Visual Sudoku）任务中，AS2 在 1,000 个测试棋盘上实现了 99.89% 的单元格准确率和 100% 的约束满足率（经 Clingo 验证），且使用的是无需外部求解器的贪婪约束解码程序。在 $N \in \{2, 4, 8\}$ 个加数的 MNIST 加法任务中，AS2 在所有规模下均实现了 99.7% 以上的数字准确率。这些结果表明，软可微不动点算子结合约束感知注意力和声明式约束规范，可以在保持完全端到端可微性的同时，达到或超过流水线式及基于求解器的神经符号系统。
