...
title_zh: 基于 MCTS 的实用查询优化：一项可复现性研究及针对复杂查询的新 MCTS 算法
...
motivation: 现有的学习型MCTS查询优化方法因代价模型的分布外泛化误差，导致其在处理真实世界多样化负载时表现不稳定。
method: 提出一种利用数据库标准内部代价模型并结合新型Extreme UCT选择策略的MCTS框架，以替代不稳定的学习组件。
result: 在JOB和JOB-Complex基准测试中，该方法在复杂连接场景下的表现超越了AlphaJoin、HyperQO及SOTA查询优化器。
conclusion: 将传统代价模型与改进的MCTS搜索策略相结合，是提升复杂查询优化鲁棒性和实用性的有效方案。
---

## 摘要
蒙特卡洛树搜索（MCTS）已被提议作为数据库查询处理中连接顺序优化的一种变革性方法，最近的框架（如 AlphaJoin 和 HyperQO）声称其性能优于传统方法。然而，这些框架依赖于学习型代价模型（learned cost models），这引发了关于泛化能力和部署就绪性的担忧。本文对这些方法进行了全面的可复现性研究，揭示了在面对多样化的工作负载时，它们往往无法维持所声称的性能提升。通过消融实验，我们诊断出这种不稳定性的根本原因：虽然 MCTS 搜索策略是有效的，但配套的学习型代价模型存在严重的分布外（out-of-distribution）泛化误差。针对这一问题，我们提出了一种新型 MCTS 框架。与以往依赖不稳定学习组件的方法不同，我们的方法利用数据库标准的内部代价模型，并辅以一种新的 Extreme UCT（应用于树的上限置信区间）选择策略，以更稳健地探索搜索空间。我们使用著名的连接顺序基准测试（JOB）和新的 JOB-Complex 基准测试，将我们的方法与原始的 AlphaJoin 和 HyperQO，以及包括动态规划（DP）和遗传查询优化（GEQO）在内的行业标准基准进行了对比测试。结果表明，在真实数据的复杂连接场景下，我们的方法优于学习型 MCTS 方法，并超越了最先进的（SOTA）查询优化器。我们发布了完整的实现和实验产物，以支持进一步的研究。
