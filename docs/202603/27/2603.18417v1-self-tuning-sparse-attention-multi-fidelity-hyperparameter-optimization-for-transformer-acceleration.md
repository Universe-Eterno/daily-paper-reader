...
tags: ["query:nco-llm"]
score: 6.0
evidence: 用于Transformer超参数调优的贝叶斯优化
tldr: 针对稀疏注意力机制中超参数难以手动调节的问题，本文提出AFBS-BO自动化框架。该框架结合贝叶斯优化与二分搜索，通过多保真度评估实现层级和头级的超参数自动优化。在Llama-2-7B上的实验表明，该方法显著提升了超参数搜索效率，并在保持模型性能的同时实现了高稀疏度加速，使稀疏注意力成为即插即用的优化工具。
selection_source: fresh_fetch
motivation: 稀疏注意力机制的超参数在不同层和模型间差异巨大，且目前依赖低效的人工网格搜索。
method: 提出AFBS-BO框架，结合贝叶斯优化进行全局探索、二分搜索进行局部精炼，并利用多保真度评估降低调优成本。
result: 在Llama-2-7B上，搜索速度提升3.4倍，评估次数减少8.8倍，且在保持密集注意力质量的同时实现了高稀疏度。
conclusion: AFBS-BO将稀疏注意力从手动调优的启发式方法转变为自优化原语，实现了跨架构的即插即用加速。
---
