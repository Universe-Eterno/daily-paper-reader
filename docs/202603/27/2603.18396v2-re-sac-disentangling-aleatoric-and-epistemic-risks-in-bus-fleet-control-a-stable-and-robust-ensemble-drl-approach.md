...
tags: ["query:nco"]
score: 7.0
evidence: 用于公交车队控制和随机优化的深度强化学习
tldr: 针对公交车辆驻留控制中随机交通和乘客需求带来的不确定性，本文提出了一种鲁棒集成软演员-评论家（RE-SAC）框架。该方法通过显式解耦偶然不确定性（噪声）和认知不确定性（数据不足），利用基于IPM的权重正则化和多样化Q集成，解决了标准DRL在波动环境下的Q值不稳定和策略崩溃问题，显著提升了公交调度的鲁棒性和效率。
...
result: "在双向公交走廊仿真中，RE-SAC的累积回报显著优于基准算法，且在稀有分布外状态下的Q值估计误差降低了高达62%。"
