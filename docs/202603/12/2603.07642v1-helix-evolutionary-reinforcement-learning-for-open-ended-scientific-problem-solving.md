...
tags: ["query:nco-llm"]
score: 8.0
evidence: 结合上下文经验的分层进化强化学习框架，用于大模型推理
...
motivation: 现有大模型在解决领域特定且开放的科学问题时，面临探索空间巨大、效率受限及泛化能力不足的问题。
method: 提出HELIX框架，通过上下文学习生成多样化候选解，并结合强化学习实现解质量的持续迭代提升。
result: 在圆堆积任务中仅凭14B模型即达到SOTA，并在机器学习基准测试中比GPT-4o平均提升了5.95个F1点。
conclusion: HELIX通过进化强化学习有效提升了模型在复杂科学任务中的探索深度与解题质量，具有极强的应用潜力。
---

## 摘要
具备推理能力的大语言模型（LLMs）在解决复杂科学问题方面展现出日益增长的潜力。然而，此类任务本质上具有领域特定性、无界性和开放性，要求在广阔且灵活的解空间中进行探索。现有方法无论是纯学习驱动还是依赖精心设计的工作流，往往面临探索效率有限和泛化能力差的问题。为克服这些挑战，我们提出了 HELIX——一种结合上下文经验的分层演化强化学习框架。HELIX 引入了两个关键创新点：(i) 一个多样化且高质量的候选解池，通过上下文学习拓宽探索范围；(ii) 强化学习用于迭代策略优化，逐步提升解的质量。这种协同作用使得发现更先进的解成为可能。在圆堆积（circle packing）任务中，HELIX 仅使用 14B 模型就实现了 2.63598308 的半径之和，达到了当前最先进的水平。在标准机器学习基准测试中，HELIX 进一步超越了采用精心设计流水线的 GPT-4o，在 Adult 和 Bank Marketing 数据集上平均 F1 分数提升了 5.95 点。
