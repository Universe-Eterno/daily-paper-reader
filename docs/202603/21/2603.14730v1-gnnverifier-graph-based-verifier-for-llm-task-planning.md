...
tldr: 针对大语言模型在任务规划中容易产生幻觉且难以识别结构性错误的问题，本文提出了GNNVerifier。该方法将规划方案表示为包含丰富属性的有向图，利用图神经网络（GNN）进行结构化评估和细粒度错误定位。通过自动生成的扰动数据进行训练，GNNVerifier能有效指导LLM进行局部修正。实验证明，该方法在多种数据集和模型上显著提升了规划质量和可靠性。
selection_source: fresh_fetch
motivation: 现有的基于LLM的验证器容易被虚假叙述误导，且难以检测任务步骤间复杂的结构性依赖错误。
method: 将规划建模为有向图，利用GNN生成全局合理性评分及局部风险评分，并据此引导LLM进行局部编辑修正。
result: 在多个基准数据集和不同LLM骨干网络上的实验均证明，GNNVerifier能显著增强规划器的性能。
conclusion: 通过引入图结构分析，GNNVerifier为LLM任务规划提供了一种比传统自反思方法更精准、更具解释性的验证机制。
---
