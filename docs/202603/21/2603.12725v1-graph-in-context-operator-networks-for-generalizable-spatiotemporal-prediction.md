...
tldr: 本研究探讨了上下文算子学习在时空预测中的应用，填补了其与传统单算子学习在相同训练条件下对比的空白。提出了 GICON（图上下文算子网络），通过图消息传递和示例感知位置编码，实现了对不同几何结构和样本数量的泛化。实验表明，GICON 在空气质量预测任务中表现优异，能够跨空间域泛化，并随推理示例增加而稳健提升性能，证明了该范式在处理复杂时空系统时的优越性。
selection_source: fresh_fetch
motivation: 旨在系统性对比上下文算子学习与传统单算子学习在时空预测任务中的性能差异。
method: 提出 GICON 模型，结合图消息传递以实现几何泛化，并采用示例感知位置编码以支持不同数量的上下文示例。
result: 在空气质量预测实验中，GICON 在复杂任务上优于传统模型，并展现出跨空间域的泛化能力和良好的扩展性。
conclusion: 上下文算子学习在处理复杂时空系统时具有显著优势，是实现通用化时空预测的有效途径。
---

## 摘要
上下文算子学习使神经网络能够从上下文示例中推断解算子，而无需更新权重。虽然先前的研究已经证明了这种范式在利用海量数据集方面的有效性，但仍缺乏与使用相同训练数据的单算子学习之间的系统比较。我们通过受控实验填补了这一空白，在相同的训练步数和数据集下，将上下文算子学习与经典算子学习（即在没有上下文示例的情况下训练的单算子模型）进行了对比。为了在真实世界的时空系统上进行这项研究，我们提出了 GICON（图上下文算子网络），它结合了用于几何泛化的图消息传递和用于基数泛化的示例感知位置编码。在中国两个地区的空气质量预测实验表明，上下文算子学习在复杂任务上优于经典算子学习，能够跨空间域泛化，并在推理时能从少量训练示例稳健地扩展到 100 个示例。
