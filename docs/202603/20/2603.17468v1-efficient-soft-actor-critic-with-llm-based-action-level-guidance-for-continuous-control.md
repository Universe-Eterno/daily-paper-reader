...
tags: ["query:nco-llm"]
score: 8.0
evidence: 基于大模型的强化学习动作级引导
tldr: 针对强化学习在广阔状态动作空间中探索效率低的问题，本文提出GuidedSAC算法。该方法创新性地利用大语言模型（LLM）作为智能监督者，通过分析轨迹状态和视觉回放提供动作级引导，干预Soft Actor-Critic（SAC）的探索过程。理论证明该方法在保留SAC收敛保证的同时提升了收敛速度。实验表明，GuidedSAC在MuJoCo等复杂任务中显著优于标准SAC及RND、ICM等主流探索增强算法，展现了极高的采样效率。
selection_source: fresh_fetch
motivation: 旨在解决传统强化学习算法在复杂连续控制任务中因探索效率低下而导致的采样效率低和性能受限问题。
method: 提出一种将LLM作为动作级监督者的框架，通过分析历史轨迹对SAC的探索过程进行智能干预和引导。
result: 在离散和连续控制基准测试中，GuidedSAC在采样效率和最终性能上均显著优于SAC及其多种先进的探索增强变体。
conclusion: 研究证明了利用LLM提供动作级引导能有效加速强化学习的探索过程，为提升复杂环境下的控制性能提供了新途径。
---
