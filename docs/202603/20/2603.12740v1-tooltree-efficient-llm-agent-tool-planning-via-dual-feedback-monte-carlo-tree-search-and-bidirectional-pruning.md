...
## 摘要
大语言模型（LLM）智能体越来越多地应用于复杂的、多步骤的任务，这些任务需要与跨多个领域的各种外部工具进行交互。然而，目前的 LLM 智能体工具规划方法通常依赖于贪婪的、反应式的工具选择策略，缺乏前瞻性，且未能考虑工具间的依赖关系。在本文中，我们提出了 ToolTree，一种受蒙特卡洛树搜索启发的新型工具规划范式。ToolTree 利用双阶段 LLM 评估和双向剪枝机制探索可能的工具使用轨迹，使智能体能够在长工具使用序列中做出明智的、自适应的决策，同时在工具执行前后剪掉不太有前景的分支。在 4 个基准测试中对开集和闭集工具规划任务进行的实证评估表明，ToolTree 在保持最高效率的同时持续提升性能，与最先进的规划范式相比，平均增益约为 10%。
