...
tags: ["query:nco-llm"]
score: 6.0
evidence: 通过双层优化进行大语言模型知识编辑
tldr: 针对大语言模型知识编辑中的“语义-执行脱节”问题，本文提出MetaKE框架。该框架将知识编辑建模为双层优化问题，将编辑目标视为可学习的元参数，通过上层优化寻找可行目标，下层执行编辑。引入结构梯度代理解决了复杂求解器的微分难题，确保编辑方向与模型流形对齐。实验证明MetaKE在多项指标上显著优于现有方法，为知识编辑提供了新视角。
selection_source: fresh_fetch
motivation: 现有知识编辑方法存在语义目标与模型可行域不匹配的问题，导致梯度截断和编辑失败。
method: 提出MetaKE框架，利用双层优化将编辑目标设为元参数，并结合结构梯度代理实现端到端优化。
result: 实验结果显示，MetaKE在多项基准测试中显著优于现有强基准方法。
conclusion: MetaKE通过将编辑目标与模型可行流形自动对齐，有效解决了知识编辑中的语义与执行失配问题。
---
