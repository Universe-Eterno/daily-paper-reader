...
tags: ["query:nco-llm"]
score: 6.0
evidence: 基于大模型的智能体通过工具调用优化决策轨迹
...
## 摘要
基于大语言模型的推荐智能体为推荐系统提供了一种极具前景的范式。然而，现有的推荐智能体通常面临中间推理与最终排序反馈脱节的问题，且无法捕捉细粒度的偏好。为此，我们提出了 AgenticRec，这是一个面向排序的智能体推荐框架，可在稀疏隐式反馈下优化整个决策轨迹（包括中间推理、工具调用和最终排序列表生成）。本研究具有三个核心贡献：首先，我们设计了一套集成在 ReAct 循环中的推荐专用工具，以支持基于证据的推理；其次，我们提出了理论上无偏的列表级组相对策略优化（list-wise GRPO），以最大化排序效用，确保复杂工具使用轨迹下的准确信用分配；第三，我们引入了渐进式偏好细化（PPR）来解决细粒度的偏好歧义。通过从排序违规中挖掘难负样本并应用双向偏好对齐，PPR 最小化了成对排序误差的凸上界。基准测试实验证实，AgenticRec 显著优于现有基准，验证了统一推理、工具使用和排序优化的必要性。
