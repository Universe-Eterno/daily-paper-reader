---
title: Set Transformer-Based Beamforming Design for Cell-Free Integrated Sensing and Communication
title_zh: 基于 Set Transformer 的无小区集成感知与通信波束赋形设计
authors: "Ranga Kulathunga, Diluka Galappaththige, Gayan Aruma Baduge, Chintha Tellambura"
date: 2026-03-24
pdf: "https://arxiv.org/pdf/2603.23618v1"
tags: ["query:nco"]
score: 7.0
evidence: 用于波束成形优化的Set Transformer
tldr: 本研究针对无小区集成感知与通信（CF-ISAC）中的波束赋形设计，提出了一种基于Set Transformer的框架（STCIB）。该框架利用注意力机制捕捉分布式接入点、用户和目标之间的全局交互，具有置换不变性且支持无监督学习。实验表明，STCIB在感知、通信及联合优化场景下，均能以极低的计算开销实现优于传统优化算法和CNN基准的性能，显著提升了系统的实时性与扩展性。
selection_source: fresh_fetch
motivation: 传统优化算法计算复杂度高且扩展性受限，而现有深度学习方法难以有效建模分布式网络实体间的长程依赖和全局交互。
method: 提出STCIB框架，利用Set Transformer的注意力机制处理无序输入并保持置换不变性，通过无监督学习实现感知与通信的联合波束赋形。
result: "实验显示STCIB在性能上优于CNN基准，且运行时间仅为传统优化算法的0.1%至0.3%，同时在联合优化场景下显著提升了感知和通信速率。"
conclusion: 该研究验证了Set Transformer在处理复杂无线网络全局关系方面的卓越能力，为实现高效、实时的CF-ISAC波束赋形提供了创新的深度学习方案。
---

## 摘要
现有的无小区集成感知与通信（CF-ISAC）波束赋形算法主要依赖于经典的优化技术，这通常会导致较高的计算复杂度和有限的可扩展性。同时，最近基于学习的方法难以捕捉分布式接入点（AP）、通信用户和感知目标之间的全局交互和长程依赖关系。为了解决这些局限性，我们提出了首个基于 Set Transformer 的 CF-ISAC 波束赋形框架（STCIB）。通过利用注意力机制，STCIB 显式地建模了网络实体之间的全局关系，能够自然地处理无序输入集，并保持了 AP、用户和目标之间的置换不变性。所提出的框架以无监督方式运行，无需标注训练数据，并支持三种设计方案：（i）以感知为中心，（ii）以通信为中心，以及（iii）ISAC 联合优化。我们将 STCIB 与卷积神经网络（CNN）基准以及两种最先进的优化算法进行了对比：凸凹过程算法（CCPA）和增广拉格朗日流形优化（ALM-MO）。数值结果表明，STCIB 的性能始终优于 CNN，在仅增加微不足道的运行时间的情况下，实现了显著更高的 ISAC 性能。例如，在方案（iii）中，当 η=0.4 时，STCIB 相比 CNN 分别将感知和通信的总速率提高了 14.8% 和 31.6%，而运行时间仅增加了 0.26%。与 CCPA 和 ALM-MO 相比，STCIB 在保持适度性能增益的同时，显著降低了计算成本。在方案（i）中，对于 3.0 bps/Hz 的通信阈值，STCIB 的运行时间仅分别为 CCPA 和 ALM-MO 所需时间的 0.1% 和 0.3%，同时将感知总速率提高了 4.45% 和 5.9%。

## Abstract
Existing cell-free integrated sensing and communication (CF-ISAC) beamforming algorithms predominantly rely on classical optimization techniques, which often entail high computational complexity and limited scalability. Meanwhile, recent learning-based approaches have difficulty capturing the global interactions and long-range dependencies among distributed access points (APs), communication users, and sensing targets. To address these limitations, we propose the first Set Transformer-based CF-ISAC beamforming framework (STCIB). By exploiting attention mechanisms, STCIB explicitly models global relationships among network entities, naturally handles unordered input sets, and preserves permutation invariance across APs, users, and targets. The proposed framework operates in an unsupervised manner, eliminating the need for labeled training data, and supports three design regimes: (i) sensing-centric, (ii) communication-centric, and (iii) joint ISAC optimization. We benchmark STCIB against a convolutional neural network (CNN) baseline and two state-of-the-art optimization algorithms: the convex-concave procedure algorithm (CCPA) and augmented Lagrangian manifold optimization (ALM-MO). Numerical results demonstrate that STCIB consistently outperforms the CNN, achieving substantially higher ISAC performance with only a negligible increase in runtime. For instance, in regime (iii), at $η$=0.4, STCIB improves the sensing and communication sum rates by 14.8 % and 31.6 %, respectively, relative to the CNN, while increasing runtime by only 0.26 %. Compared with CCPA and ALM-MO, STCIB offers significantly lower computational cost while maintaining modest performance gains. In regime (i), for a 3.0 bps/Hz communication threshold, the runtime of STCIB is only 0.1 % and 0.3 % of that required by CCPA and ALM-MO, respectively, while improving the sensing sum rate by 4.45 % and 5.9 %.