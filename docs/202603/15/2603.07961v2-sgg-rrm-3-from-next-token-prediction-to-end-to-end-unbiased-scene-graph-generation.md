...
## 摘要
场景图生成 (SGG) 将视觉场景结构化为物体及其关系的图。尽管多模态大语言模型 (MLLMs) 推动了端到端 SGG 的发展，但当前方法仍受限于缺乏特定任务的结构化推理，以及稀疏且长尾的关系分布挑战，导致生成的场景图不完整，表现为低召回率和有偏预测。为了解决这些问题，我们引入了 SGG-R$^{\rm 3}$，这是一个结构化推理框架，它集成了特定任务的思维链 (CoT) 引导的有监督微调 (SFT) 以及结合组序列策略优化 (GSPO) 的强化学习 (RL)，旨在通过三个连续阶段实现端到端的无偏场景图生成。在 SFT 阶段，我们提出了一种关系增强策略，利用 MLLM 并通过嵌入相似度过滤进行精炼，以缓解关系稀疏性。随后，一种阶段对齐的奖励方案优化了 RL 过程中的程序化推理。具体而言，我们提出了一种新颖的双粒度奖励，整合了细粒度和粗粒度的关系奖励，同时通过基于频率的谓词自适应加权减轻长尾问题，并通过语义聚类提高关系覆盖率。在两个基准测试上的实验表明，SGG-R$^{\rm 3}$ 与现有方法相比取得了优越的性能，证明了该框架的有效性和泛化能力。
