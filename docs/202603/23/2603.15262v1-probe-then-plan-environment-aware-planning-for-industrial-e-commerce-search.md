...
motivation: 现有LLM搜索范式面临查询重写脱离实时库存与深度代理迭代延迟过高的双重挑战。
method: 引入“先探测后规划”机制，通过轻量级探测获取检索快照，并结合教师代理引导的SFT与强化学习优化规划器。
result: 在京东商城的在线A/B测试中，该方法显著提升了相关召回率、用户转化率（UCVR）和商品交易总额（GMV）。
conclusion: EASP通过环境感知与自适应调度，有效平衡了搜索规划的准确性与工业级实时性要求。
---
