...
## 摘要
大型深度神经网络（DNN），尤其是基于 Transformer 和多模态架构的网络，计算需求巨大，在野外机器人等资源受限的边缘平台上部署具有挑战性。在任务关键型场景（如灾难响应）中，这些挑战尤为突出，机器人必须在带宽、延迟和电池寿命的严格约束下进行协作，且通常缺乏基础设施或服务器支持。为应对这些限制，我们提出了 COHORT，这是一个基于机器人操作系统（ROS）的多机器人系统协作 DNN 推理与任务执行框架。COHORT 采用混合离线-在线强化学习（RL）策略，在机器人之间动态调度和分配 DNN 模块的执行。我们的主要贡献包括三个方面：(a) 结合优势加权回归（AWR）的离线 RL 策略学习，利用分布式机器人异构 DNN 工作负载的基于拍卖的任务分配数据进行训练；(b) 通过多智能体 PPO（MAPPO）进行在线策略自适应，由离线策略初始化并实时微调；(c) 在 CLIP 和 SAM 等视觉语言模型（VLM）推理任务上对 COHORT 进行全面评估，分析其在机器人/工作负载增加时的可扩展性以及鲁棒性。我们将 COHORT 与遗传算法及多个 RL 基准进行了对比。实验结果表明，COHORT 将电池消耗降低了 15.4%，GPU 利用率提高了 51.67%，同时满足帧率和截止时间约束的次数达到了对比方案的 2.55 倍。
