...
tldr: 本研究提出了Q-BIOLAT框架，旨在通过二进制潜空间建模和优化蛋白质适应度景观。该方法利用预训练蛋白质语言模型获取嵌入，并将其转化为紧凑的二进制表示，随后通过二次无约束二进制优化（QUBO）模型近似适应度。实验表明，该框架在ProteinGym基准上能有效识别高适应度变体，并为量子辅助蛋白质工程提供了与量子退火硬件兼容的新路径。
selection_source: fresh_fetch
motivation: 为了克服蛋白质序列空间巨大的搜索挑战，并探索量子计算在蛋白质工程中的应用潜力。
method: 将蛋白质语言模型的连续嵌入转化为二进制潜空间，并构建QUBO模型以支持经典启发式及量子退火优化。
result: 实验证明Q-BIOLAT能有效捕捉适应度景观结构，在ProteinGym基准下成功检索到高适应度的蛋白质变体。
conclusion: 该框架实现了蛋白质表示学习与组合优化的有机结合，为未来利用量子硬件进行蛋白质设计奠定了基础。
---
