...
tldr: 针对自然语言提示词在处理多目标任务时存在的歧义性问题，本文提出了 UtilityMax Prompting 框架。该框架将任务重构为以 LLM 回答为决策变量的影响图，并定义效用函数，引导模型通过最大化期望效用进行推理。在 MovieLens 1M 数据集上的实验表明，该方法在多目标推荐任务中显著提升了精确度和 NDCG，为 LLM 优化提供了形式化路径。
selection_source: fresh_fetch
motivation: 自然语言提示词在处理需要同时满足多个目标的复杂任务时具有固有的模糊性。
method: 将任务建模为影响图，通过定义效用函数并要求 LLM 最大化期望效用来实现精确的目标优化。
result: 在 MovieLens 1M 数据集上，该方法在 Claude、GPT 和 Gemini 等主流模型中均取得了优于自然语言基线的精确度和 NDCG 表现。
conclusion: 采用形式化数学语言代替自然语言进行提示，能有效约束 LLM 的推理过程，实现更精准的多目标优化。
---

## 摘要
大语言模型（LLM）任务的成功在很大程度上取决于其提示词（prompt）。大多数用例使用自然语言来指定提示词，而当需要同时满足多个目标时，自然语言本质上具有歧义性。在本文中，我们介绍了 UtilityMax Prompting，这是一个使用形式化数学语言指定任务的框架。我们将任务重构为一个影响图（influence diagram），其中 LLM 的回答是唯一的决策变量。在该图的条件概率分布上定义了一个效用函数，并指示 LLM 寻找使期望效用最大化的回答。这约束了 LLM 对目标的每个组成部分进行显式推理，将其输出导向精确的优化目标，而非主观的自然语言解释。我们在 MovieLens 1M 数据集上，针对三种前沿模型（Claude Sonnet 4.6、GPT-5.4 和 Gemini 2.5 Pro）验证了我们的方法，证明了在多目标电影推荐任务中，该方法在准确率（precision）和归一化折损累计增益（NDCG）方面相较于自然语言基准具有持续的提升。
