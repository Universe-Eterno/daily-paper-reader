...
## 摘要
穿过狭窄通道的运动规划仍然是一个核心挑战：基于采样的规划器很少在这些狭窄但关键的区域内放置样本，且即使样本落在通道内，它们之间的直线连接也因靠近障碍物边界而经常被碰撞检测拒绝。基于分解的规划器通过将自由空间划分为凸单元来解决这两个问题——每个通道都被精确地捕获为单元边界，且单元内的任何路径在构造上都是无碰撞的。然而，通过单元图的候选走廊数量随环境复杂度呈组合式增长，从而在走廊选择中形成了瓶颈。我们提出了 GNN-DIP，这是一个通过将图神经网络（GNN）与两阶段分解启发式规划器（DIP）相结合来解决该问题的框架。GNN 在单元邻接图上预测门户得分，以引导走廊搜索偏向近优区域，同时保持完备性。在二维空间中，结合漏斗算法的约束德劳内三角剖分（CDT）可在走廊内产生精确的最短路径；在三维空间中，带有门户面采样的 Slab 凸分解提供了近优的路径评估。在二维狭窄通道场景、包含多达 246 个障碍物的三维瓶颈环境以及动态二维设置下的基准测试表明，GNN-DIP 实现了 99–100% 的成功率，且比基于采样的基准方法提速 2–280 倍。
