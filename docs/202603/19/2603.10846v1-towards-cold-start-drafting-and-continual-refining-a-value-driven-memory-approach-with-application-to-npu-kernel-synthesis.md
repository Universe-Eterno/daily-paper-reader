...
tldr: 针对新兴NPU架构中内核合成面临的“数据墙”和冷启动挑战，本文提出EvoKernel框架。该框架采用基于价值驱动记忆的强化学习方法，通过自演进的智能体实现从初始草案生成到持续性能优化的全生命周期自动化。EvoKernel利用Q值检索机制和跨任务记忆共享，显著提升了通用大模型在数据稀缺领域的代码生成正确率和执行效率。
selection_source: fresh_fetch
motivation: 解决大语言模型在数据稀缺的NPU编程领域因缺乏训练数据而导致的性能大幅下降和冷启动问题。
method: 提出一种名为EvoKernel的自演进智能体框架，通过价值驱动的记忆检索机制和强化学习，实现内核代码的阶段性生成与迭代优化。
result: "在NPU内核基准测试中，将模型的正确率从11.0%提升至83.0%，并实现了中位数3.60倍的执行加速。"
conclusion: 价值引导的经验积累能够使通用模型在缺乏大规模数据的利基硬件生态中高效完成内核合成任务。
---

## 摘要
将大语言模型部署到数据稀缺的编程领域面临着重大挑战，特别是在新兴的领域特定架构（DSA）上的算子内核合成，其中“数据墙”限制了可用的训练数据。虽然模型在 CUDA 等数据丰富的平台上表现出色，但在 NPU 编程等数据稀缺的生态系统中，其性能会出现灾难性的下降。为了在不进行昂贵微调的情况下克服这一冷启动障碍，我们引入了 EvoKernel，这是一个自我演进的智能体框架，能够自动化从初始草拟到持续精炼的算子内核合成全生命周期。EvoKernel 通过将合成过程建模为基于记忆的强化学习任务来解决这一问题。通过一种新颖的价值驱动检索机制，它学习特定阶段的 Q 值，根据经验对当前目标的贡献（无论是引导生成可行的草案还是迭代优化延迟）来确定经验的优先级。此外，通过启用跨任务记忆共享，该智能体能够将从简单算子中获得的见解推广到复杂算子。通过构建 KernelBench 的 NPU 变体并在其上进行评估，EvoKernel 将前沿模型的正确率从 11.0% 提高到 83.0%，并通过迭代精炼实现了相比初始草案 3.60 倍的中值加速。这表明，价值引导的经验积累使通用模型能够掌握小众硬件生态系统中的算子内核合成任务。我们的官方页面位于 https://evokernel.zhuo.li。
