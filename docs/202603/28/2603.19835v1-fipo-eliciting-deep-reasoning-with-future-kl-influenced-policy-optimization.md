...
motivation: 传统基于结果的奖励机制将奖励均匀分配给所有token，导致信用分配不均，难以区分关键逻辑转折点与琐碎token。
method: 提出FIPO算法，通过将折扣未来KL散度整合进策略更新，根据token对后续轨迹的影响力重新分配权重，实现稠密优势计算。
result: "在Qwen2.5-32B上，FIPO将思维链长度从4,000提升至10,000个token以上，并将AIME 2024的Pass@1准确率从50%提升至最高58%。"
conclusion: 建立稠密优势表述是优化基于结果奖励的算法、释放基础模型深度推理潜力的关键路径。
---
