...
tldr: 本研究提出 RouterKGQA 框架，旨在解决知识图谱问答（KGQA）中专用小模型效率高但易出错、通用大模型推理强但成本高的问题。该框架通过路由机制实现专用与通用模型的协作：专用模型负责生成推理路径并进行约束感知过滤，仅在必要时由通用模型进行修复。实验表明，该方法在提升准确率的同时显著降低了推理成本，为高效、准确的 KGQA 提供了新方案。
selection_source: fresh_fetch
motivation: 现有的 KGQA 方法在专用模型的运行效率与通用大模型的结构化推理能力及约束处理能力之间难以取得平衡。
method: 提出一种专用-通用模型协作框架，由专用模型生成路径并进行约束感知过滤，仅在必要时触发通用模型进行修复。
result: "在多个基准测试中 F1 值平均提升 3.57，Hits@1 提升 0.49，且平均每题仅需 1.15 次大模型调用。"
conclusion: RouterKGQA 通过灵活的模型路由和高效的工作流设计，实现了高性能且低成本的约束感知知识图谱问答。
---
