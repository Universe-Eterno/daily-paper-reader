...
## 摘要
机器人的智能行为并非仅源于控制系统，而是源于身体与大脑之间的紧密耦合，这一原理被称为具身智能（embodied intelligence）。设计能够利用这种相互作用的软体机器人仍然是一个重大挑战，特别是当形态和控制需要同时优化时。在这种协同设计过程中，一个主要障碍是形态演化可能会破坏已学习的控制策略，导致现有知识难以重用或调整。我们通过开发一种基于图神经网络的方法来解决这一问题，用于形态和控制器的协同设计。每个机器人被表示为一个图，利用图注意力网络（GAT）对节点特征进行编码，并将池化后的表示通过多层感知机（MLP）头部，以产生执行器指令或价值估计。在演化过程中，继承遵循拓扑一致的映射：重用共享的 GAT 层，完整迁移 MLP 隐藏层，复制匹配的执行器输出，并对不匹配的输出进行随机初始化和微调。这种形态感知的策略类使得控制器能够在身体发生变异时进行自适应。在基准测试中，与传统的仅使用 MLP 的协同设计方法相比，我们基于 GAT 的方法实现了更高的最终适应度，并对形态变化表现出更强的适应性。这些结果表明，图结构策略为演化形态与具身智能控制之间提供了更有效的接口。
