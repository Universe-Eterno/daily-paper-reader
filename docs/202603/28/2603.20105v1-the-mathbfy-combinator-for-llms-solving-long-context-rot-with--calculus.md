...
## 摘要
大语言模型（LLM）越来越多地被用作通用推理器，但长输入仍受限于固定的上下文窗口瓶颈。递归语言模型（RLM）通过将提示词外部化并递归地解决子问题来应对这一挑战。然而，现有的 RLM 依赖于开放式的读取-求值-打印循环（REPL），模型在其中生成任意控制代码，导致执行过程难以验证、预测和分析。我们提出了 $\lambda$-RLM，这是一个
