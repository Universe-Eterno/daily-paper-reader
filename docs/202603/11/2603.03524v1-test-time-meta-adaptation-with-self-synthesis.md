...
tldr: 本研究针对大语言模型在推理阶段适应多样化任务的需求，提出了MASS元学习框架。该框架通过自合成（Self-Synthesis）生成特定问题的训练数据，并利用双层优化机制进行测试时自适应。内循环在合成数据上更新模型，外循环通过元梯度优化数据生成质量，以提升下游任务表现。在数学推理实验中，MASS成功实现了高效且针对性强的测试时自我改进。
selection_source: fresh_fetch
motivation: 大语言模型在面对多样化领域和任务时，需要具备在推理阶段进行自我调整和持续改进的能力。
method: 提出MASS框架，通过双层优化和元梯度技术，让模型学习如何生成高质量合成数据并据此进行测试时自更新。
result: 在数学推理任务上的实验证明，MASS能够生成针对性强的课程数据，实现高效且数据节省的测试时自适应。
conclusion: MASS为大语言模型提供了一种有效的元学习机制，使其能够通过自合成数据在推理阶段实现性能的自我提升。
---
