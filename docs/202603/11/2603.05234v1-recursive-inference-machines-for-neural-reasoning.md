...
## 摘要
诸如微型递归模型（TRMs）之类的神经推理器通过将神经骨干网络与专门的推理方案相结合来解决复杂问题。此类推理方案一直是随机推理系统的核心组成部分，在这些系统中，推理规则被应用于随机模型，以推导出复杂查询的答案。在这项工作中，我们通过引入递归推理机（RIMs）来桥接这两个范式。RIMs 是一种神经推理框架，它明确地结合了受经典推理引擎启发的递归推理机制。我们证明了 TRMs 可以被表示为 RIMs 的一个实例，这使我们能够通过重加权组件对其进行扩展，从而在包括 ARC-AGI-1、ARC-AGI-2 和 Sudoku Extreme 在内的具有挑战性的推理基准测试中获得更好的性能。此外，我们还展示了 RIMs 可用于改进其他任务的推理，例如表格数据的分类，其表现优于 TabPFNs。
