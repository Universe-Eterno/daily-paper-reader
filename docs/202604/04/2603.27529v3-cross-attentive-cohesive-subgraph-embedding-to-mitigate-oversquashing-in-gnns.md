...
title_zh: 缓解图神经网络中过度挤压的交叉注意力凝聚子图嵌入
...
tldr: 针对图神经网络（GNN）在处理长程依赖时存在的“过度挤压”问题，本文提出一种基于交叉注意力内聚子图嵌入的新型框架。该方法通过提取长程信息中的内聚结构并利用交叉注意力机制增强节点表示，在过滤噪声连接的同时保留关键全局上下文，从而有效缓解信息传递中的瓶颈限制。实验证明，该模型在多个基准数据集的分类任务中均取得了显著的性能提升。
selection_source: fresh_fetch
motivation: 解决GNN在处理长程信息时因信息压缩导致的过度挤压（oversquashing）及全局上下文丢失问题。
method: 提出一种利用交叉注意力机制融合内聚子图表示的框架，通过强化长程内聚结构并过滤无关连接来增强节点嵌入。
result: 在多个基准数据集上的实验结果显示，该模型在分类准确率上显著且一致地优于现有基线方法。
conclusion: 该研究证明了通过引入内聚子图结构能有效缓解过度挤压，提升GNN捕捉全局信息的能力。
---

## 摘要
图神经网络 (GNNs) 在各种现实领域中都取得了出色的性能。然而，它们面临着过度挤压 (oversquashing) 的问题，即长距离信息在通过有限的消息传递路径进行压缩时会发生扭曲。这一瓶颈限制了它们捕获关键全局上下文的能力，并降低了其性能，特别是在图的密集和异质区域。为了解决这一问题，我们提出了一种新颖的图学习框架，该框架通过交叉注意力凝聚子图表示来丰富节点嵌入，从而减轻过度长距离依赖的影响。该框架通过强调长距离信息中的凝聚结构并移除噪声或无关连接来增强节点表示。它在不使狭窄的瓶颈通道过载的情况下保留了关键的全局上下文，从而进一步缓解了过度挤压。在多个基准数据集上的广泛实验表明，与标准基准方法相比，我们的模型在分类准确率上取得了持续的提升。
