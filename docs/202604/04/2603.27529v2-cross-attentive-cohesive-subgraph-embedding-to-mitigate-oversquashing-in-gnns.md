...
tldr: 针对图神经网络在处理长程依赖时存在的过度挤压问题，本文提出了一种基于交叉注意力内聚子图嵌入的新型框架。该方法通过引入内聚子图表示来增强节点嵌入，在保留关键全局上下文的同时过滤噪声连接，有效缓解了信息传递中的瓶颈限制。实验证明，该模型在多个基准数据集上的分类准确率均优于传统基准方法，显著提升了模型捕捉全局结构的能力。
selection_source: fresh_fetch
motivation: 旨在解决GNN在处理长程信息时因通道瓶颈导致的过度挤压问题，以及在稠密和异质图中捕捉全局上下文的局限性。
method: 提出一种利用交叉注意力机制融合内聚子图表示的框架，通过强调长程信息中的内聚结构并去除无关连接来增强节点嵌入。
result: 在多个基准数据集上的实验结果显示，该模型在分类准确率上相较于标准基准方法取得了持续且显著的提升。
conclusion: 通过引入内聚子图嵌入和交叉注意力机制，可以有效缓解GNN的过度挤压效应并增强其对复杂图结构的表征能力。
---

## 摘要
图神经网络（GNNs）在多个现实领域中取得了卓越的性能。然而，它们面临着过度压缩（oversquashing）的问题，即长程信息在通过有限的消息传递路径进行压缩时会发生扭曲。这一瓶颈限制了它们捕获关键全局上下文的能力，并降低了其性能，特别是在图的密集区域和异质区域。为解决这一问题，我们提出了一种新颖的图学习框架，该框架通过交叉注意力凝聚子图表示来丰富节点嵌入，从而减轻过度长程依赖的影响。该框架通过强调长程信息中的凝聚结构并移除噪声或无关连接来增强节点表示。它在不使狭窄的瓶颈通道过载的情况下保留了关键的全局上下文，从而进一步缓解了过度压缩。在多个基准数据集上的广泛实验表明，与标准基线方法相比，我们的模型在分类准确率上取得了持续的提升。
