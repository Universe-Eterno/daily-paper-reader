...
## 摘要
本文研究了信号时序逻辑（STL）规范下的离线零样本规划。我们假设仅能访问由任务无关的行为策略收集的状态-动作-状态转移离线数据集，且没有解析动力学模型，不进行进一步的环境交互，也不进行针对特定任务的重新训练。目标是合成一种控制策略，使其生成的轨迹满足任意未见的 STL 规范。为此，我们提出了 GraSP-STL，这是一个基于图搜索的框架，用于从离线轨迹中进行零样本 STL 规划。该方法从离线数据中学习目标条件价值函数，并利用其在状态空间上导出有限时域可达性度量。基于该度量，它构建了一个有向图抽象，其节点表示代表性状态，边编码了可行的短时域转移。随后，规划被建模为对路标点序列的图搜索，使用算术-几何平均鲁棒性及其区间语义进行评估，并由学习到的目标条件策略执行。所提出的框架将可重用的可达性学习与任务条件规划分离，从而实现了对未见 STL 任务的零样本泛化，并通过组合离线数据中的短时域行为实现长时域规划。实验结果证明了该方法在一系列离线 STL 规划任务中的有效性。
