...
motivation: 在密集且复杂的自动驾驶场景中，如何实现多智能体之间高效且安全的协作仍是一个重大挑战。
method: 提出COIN框架，利用CIG-TD3算法优化个体与全局目标，并结合双层交互感知架构捕捉局部与全局依赖。
result: 在密集城市交通仿真和真实机器人实验中，COIN在安全性和效率指标上均一致优于其他先进基准方法。
conclusion: COIN通过增强交互感知和优化信用分配，为复杂动态环境下的多智能体自动驾驶提供了卓越的协作解决方案。
---

## 摘要
多智能体自动驾驶（MASD）系统为协调自动驾驶车辆提供了一种有效的解决方案，旨在减少未来智能交通系统中的拥堵，并提高安全性和运行效率。多智能体强化学习（MARL）已成为开发先进端到端 MASD 系统的一种极具前景的方法。然而，在具有复杂智能体交互的密集场景中，在动态 MASD 系统中实现高效且安全的协作仍然是一个重大挑战。为了应对这一挑战，我们提出了一种名为 COIN 的新型协作（CO-）交互感知（-IN）MARL 框架。具体而言，我们开发了一种新的反事实个体-全局双延迟深度确定性策略梯度（CIG-TD3）算法，该算法采用“中心化训练，去中心化执行”（CTDE）模式，旨在联合优化智能体的个体目标（导航）和全局目标（协作）。我们进一步引入了一种双层交互感知中心化评论家（critic）架构，该架构能够捕捉局部成对交互和全局系统级依赖关系，从而实现更准确的全局价值估计，并改进协作策略学习的信用分配。我们在密集的城市交通环境中进行了广泛的仿真实验，结果表明，在各种系统规模下，COIN 在安全性和效率方面均始终优于其他先进的基准方法。这些结果突显了其在复杂动态 MASD 场景中的优越性，并通过真实世界的机器人演示得到了进一步验证。补充视频可在 https://marmotlab.github.io/COIN/ 获取。
