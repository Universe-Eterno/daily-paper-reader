...
title_zh: 一个 Token 就够了吗？基于大语言模型的图问答中的图池化 Token
...
tldr: 本研究针对基于大语言模型（LLM）的图问答（GraphQA）中，将复杂图结构压缩为单个Token导致的严重信息瓶颈问题，提出了多Token池化策略。通过引入Top-k、VNPool等多种层次化池化算子，并结合LoRA微调技术，有效提升了图到LLM接口的传输带宽和语义质量。实验证明，该方法在WebQSP等基准上表现优异，同时揭示了当前GraphQA基准测试中存在的表示饱和现象。
selection_source: fresh_fetch
motivation: 现有的GraphQA架构通常将整个图结构压缩为单个Token，这种过度压缩造成了严重的信息瓶颈，限制了LLM对复杂图结构的理解。
method: 探索了多种层次化剪枝和聚类池化算子（如VNPool、Top-k等），通过多Token投影增加接口带宽，并利用LoRA稳定训练过程。
result: "实验表明VNPool和剪枝方法在LoRA加持下表现最稳健，在WebQSP数据集上达到了约73%的Hit@1准确率，媲美全图基线。"
conclusion: 多Token池化能有效缓解信息瓶颈，但研究也发现当前GraphQA基准存在表示饱和问题，即答案往往仅与孤立节点特征高度相关。
---

## 摘要
图神经网络 (GNN) 与大语言模型 (LLM) 的结合已成为图问答 (GraphQA) 领域一种极具前景的范式。然而，如何将复杂的结构信息有效地编码进 LLM 的潜空间仍是一个待解决的挑战。目前的先进架构（如 G-Retriever）通常依赖标准的 GNN 和激进的均值池化将整个图子结构压缩为单个 token，从而造成了严重的信息瓶颈。本研究通过探索两种正交策略来缓解这一瓶颈：(1) 通过多 token 池化增加图到 LLM 接口的带宽；(2) 通过全局注意力机制增强图编码器的语义质量。我们评估了一系列层次化剪枝和基于聚类的池化算子，包括 Top-k、SAGPool、DiffPool、MinCutPool 和虚拟节点池化 (VNPool)，旨在将图数据投影为多个可学习的 token。实验表明，虽然池化在软提示微调过程中会引入显著的不稳定性，但应用低秩自适应 (LoRA) 可以有效稳定特定的层次化投影（特别是 VNPool 和剪枝方法），尽管密集聚类算子仍然具有挑战性。这种稳定性使得压缩表示能够与全图基准相媲美（在 WebQSP 上达到约 73% 的 Hit@1）。在概念上，我们证明了采用 VNPool 实现的 Graph Transformer 在结构上等同于单层 Perceiver IO 编码器。最后，我们将 FandE（特征与边）评分适配到生成式 GraphQA 领域。我们的分析揭示了 GraphQA 基准测试存在表示饱和问题，即目标答案往往与孤立的节点特征高度相关。代码实现见 https://github.com/Agrover112/G-Retriever/tree/all_good/
