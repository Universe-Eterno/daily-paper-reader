...
title_zh: 通信延迟下的分布式在线子模最大化：一种同步决策方法
...
tldr: 针对动态环境下存在通信延迟的多智能体次模最大化问题，本文提出了一种分布式在线贪婪算法（DOG）。该算法结合了对抗性老虎机学习与延迟反馈机制，支持在任意网络拓扑下进行同步决策。研究分析了去中心化带来的次优成本与网络结构的关系，并揭示了协调性能与收敛时间之间的权衡，填补了完全中心化与局部协调方法之间的空白。
selection_source: fresh_fetch
motivation: 现有的在线次模最大化方法在处理通信延迟时，要么因顺序通信导致高延迟，要么因仅限一跳协调而限制了性能。
method: 提出了分布式在线贪婪（DOG）算法，利用带延迟反馈的对抗性老虎机学习工具实现跨任意拓扑的同步决策。
result: 理论分析给出了算法相对于最优解的近似性能，并量化了去中心化成本与网络结构及通信延迟之间的权衡关系。
conclusion: DOG 算法在性能上衔接了完全中心化和完全去中心化的一跳协调方法，为延迟环境下的分布式信息采集任务提供了有效方案。
---

## 摘要
我们针对通信延迟下的多智能体子模最大化问题，提出了一种分布式在线算法。我们的研究动机源于未知动态环境中的未来分布式信息采集任务，在这些任务中，效用函数自然地表现出边际收益递减特性，即子模性。现有的在线子模最大化方法要么依赖于顺序多跳通信，导致极高的延迟和苛刻的连通性假设；要么将每个智能体的协调限制在其一跳邻域内，从而限制了协调性能。为解决这一问题，我们提出了分布式在线贪婪（DOG）算法，该算法集成了带有延迟反馈的对抗性老虎机学习工具，以实现在任意网络拓扑结构下的同步决策。我们给出了 DOG 算法相对于最优解的近似性能，并将由于去中心化导致的次优成本描述为网络结构的函数。我们的分析进一步揭示了协调性能与收敛时间之间的权衡，这种权衡由通信延迟的大小决定。通过这种权衡，DOG 算法涵盖了从最先进的全中心化在线协调方法 [1] 到全去中心化一跳协调方法 [2] 的整个范畴。
