...
tldr: 针对多智能体检索增强生成（RAG）中静态编排和行为固化的局限性，本文提出HERA框架。该框架采用分层演化机制，在全局层面通过奖励引导优化查询特定的智能体拓扑结构，在局部层面利用角色感知提示演化精炼智能体行为。实验证明，HERA在多个知识密集型任务上显著提升了性能，实现了高效的协同推理与自组织能力。
selection_source: fresh_fetch
motivation: 现有的多智能体RAG系统因依赖固定的编排策略和静态智能体行为，在处理复杂、多跳的查询时表现出性能脆弱性。
method: 提出HERA框架，通过全局拓扑优化和局部角色感知提示演化，实现多智能体编排与个体行为的联合进化。
result: "在六个知识密集型基准测试中，HERA较最新基准平均提升了38.69%，同时保持了强大的泛化性和Token使用效率。"
conclusion: 通过经验引导的演化机制，多智能体系统能够自发形成高效的协作网络，显著增强了复杂推理任务的处理效果。
---

## 摘要
多智能体检索增强生成（RAG）通过让每个智能体承担特定角色，支持需要多步骤、多来源或复杂推理的难题。然而，现有方法依赖于静态的智能体行为和固定的编排策略，导致在多样化的多跳任务中表现脆弱。我们识别出两个关键局限性：缺乏持续自适应的编排机制，以及单个智能体缺乏行为层面的学习。为此，我们提出了 HERA，这是一个共同演化多智能体编排和特定角色智能体提示词的分层框架。在全局层面，HERA 通过奖励引导的采样和经验积累来优化特定查询的智能体拓扑结构。在局部层面，角色感知提示词演化通过信用分配以及沿操作和行为原则的双轴适配来优化智能体行为，从而实现有针对性的、基于角色的改进。在六个知识密集型基准测试中，HERA 相比近期基准模型实现了平均 38.69% 的提升，同时保持了稳健的泛化能力和 Token 效率。拓扑分析揭示了涌现的自组织现象，其中稀疏探索产生了紧凑且高实用性的多智能体网络，展示了高效的协作和稳健的推理能力。
