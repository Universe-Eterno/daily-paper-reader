...
tldr: 针对室内移动机器人目标搜索中存在的部分可观测性和感知不确定性挑战，本文提出一种结合贝叶斯推理与深度强化学习（DRL）的混合框架。该方法利用贝叶斯推理在线更新目标位置的空间置信图，并训练DRL策略基于该概率表示选择导航动作。在Habitat 3.0仿真环境中的实验表明，该方法在提高搜索成功率的同时减少了搜索开销，有效融合了概率建模的解释性与强化学习的自适应性。
selection_source: fresh_fetch
motivation: 旨在解决室内目标搜索中传统概率方法依赖手工启发式规则以及深度强化学习收敛慢、解释性差的问题。
method: 提出一种混合框架，通过贝叶斯推理维护空间置信图，并利用强化学习策略直接从该概率表示中学习导航决策。
result: 在Habitat 3.0仿真实验中，该方法相比基准策略显著提升了搜索成功率并降低了搜索路径成本。
conclusion: 结合贝叶斯置信估计与学习型动作选择，是实现部分可观测环境下高效可靠目标搜索的有效途径。
---
