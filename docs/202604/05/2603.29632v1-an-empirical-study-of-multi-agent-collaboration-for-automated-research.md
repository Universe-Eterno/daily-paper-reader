...
## 摘要
随着人工智能智能体（AI agents）的演进，研究界正迅速从单一大型语言模型（LLMs）转向多智能体系统（MAS），以克服自动化研究中的认知瓶颈。然而，针对这些自主智能体的最优多智能体协调框架在很大程度上仍未得到探索。在本文中，我们提出了一项系统的实证研究，调查了不同多智能体结构在自动化机器学习优化中的相对效能。利用一个配备了 Git 工作树隔离和显式全局内存的严格受控、基于执行的测试平台，我们将单智能体基准与两种多智能体范式进行了对比：子智能体架构（具有事后整合的并行探索）和智能体团队架构（具有执行前交接的专家协作）。通过在严格固定的计算时间预算下评估这些系统，我们的研究结果揭示了运行稳定性与理论深思（theoretical deliberation）之间的基本权衡。子智能体模式表现为一个高弹性、高吞吐量的搜索引擎，最适合在严格时间限制下进行广泛且浅层的优化。相反，由于多作者代码生成，智能体团队拓扑结构表现出更高的运行脆弱性，但在计算预算充足的情况下，它能实现复杂架构重构所需的深层理论对齐。这些实证见解为设计未来的自动化研究系统提供了可操作的指南，提倡采用动态路由架构，使其协作结构能够根据实时任务复杂度进行自适应调整。
