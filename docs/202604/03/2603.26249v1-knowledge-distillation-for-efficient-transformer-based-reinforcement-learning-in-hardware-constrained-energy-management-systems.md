...
title_zh: 硬件受限能源管理系统中高效基于 Transformer 强化学习的知识蒸馏
...
tags: ["query:nco"]
score: 7.0
evidence: 基于Transformer的强化学习用于能源管理优化
...
## 摘要
基于 Transformer 的强化学习已成为住宅能源管理中序列控制的有力候选方案。特别是，决策 Transformer（Decision Transformer）能够从历史数据中学习有效的电池调度策略，从而提高光伏自发自用率并降低电费。然而，Transformer 模型通常计算需求过高，难以部署在资源受限的住宅控制器上，而这些控制器的内存和延迟约束至关重要。本文研究了利用知识蒸馏将高容量决策 Transformer 策略的决策行为转移到更适合嵌入式部署的紧凑模型中。利用 Ausgrid 数据集，我们在离线序列化决策 Transformer 框架下，针对异构多建筑数据训练了教师模型。随后，我们通过匹配教师模型的动作来蒸馏出更小的学生模型，从而在减小模型规模的同时保持控制质量。在广泛的教师-学生配置中，蒸馏在很大程度上保留了控制性能，甚至实现了高达 1% 的小幅提升，同时减少了高达 96% 的参数量、90% 的推理内存和 63% 的推理时间。除了这些压缩效果外，当蒸馏到具有相同架构容量的学生模型时，也观察到了相当的成本改进。总之，我们的结果表明，知识蒸馏使得决策 Transformer 控制更适用于资源受限硬件上的住宅能源管理。
