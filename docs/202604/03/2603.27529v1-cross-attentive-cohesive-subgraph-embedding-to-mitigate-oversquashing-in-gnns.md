...
tags: ["query:nco"]
score: 6.0
evidence: 用于捕获图中全局上下文的GNN框架
tldr: 针对图神经网络在处理长程依赖时的过度挤压问题，本文提出一种基于交叉注意力内聚子图嵌入的框架。该方法通过引入内聚子图表示增强节点嵌入，在保留关键全局上下文的同时过滤噪声连接，有效缓解了信息传递瓶颈。实验证明，该模型在多个基准数据集上显著提升了分类准确率，为解决GNN长程信息丢失提供了新方案。
selection_source: fresh_fetch
motivation: 旨在解决GNN在处理长程信息时因信息压缩导致的过度挤压问题，及其在稠密和异质图中捕捉全局上下文的局限性。
method: 提出一种利用交叉注意力机制融合内聚子图表示的框架，通过强调长程信息中的内聚结构并去除无关连接来增强节点表示。
result: 在多个基准数据集上的实验结果显示，该模型在分类准确率上相较于标准基准方法取得了持续且显著的性能提升。
conclusion: 通过引入内聚子图嵌入和交叉注意力机制，可以有效缓解GNN的过度挤压瓶颈并增强其对复杂全局结构的表征能力。
---
